...
class Earth extends React.Component {
    ...
    animate() {
        const earth = SVG("#earth");
        const earthShine = SVG("#earthShine");
        const leftMan = SVG("#leftMan");
        const rightMan = SVG("#rightMan");
        earth.timeline().finish();
        earthShine.timeline().finish();
        leftMan.timeline().finish();
        rightMan.timeline().finish();
        if (!this.state.animated) {
            this.setState({ animated: true });
            earth.animate({ duration: 2000 }).rotate(-180);
            earthShine.animate({ duration: 2000 }).rotate(180);
            leftMan
                .animate({ duration: 1000 })
                .transform({ rotate: -5, origin: "bottom" });
            rightMan
                .animate({ duration: 1000 })
                .transform({ rotate: 5, origin: "bottom" });
        } else {
            ...
            leftMan
                .animate({ duration: 1000 })
                .transform({ rotate: 0, origin: "bottom" });
            rightMan
                .animate({ duration: 1000 })
                .transform({ rotate: 0, origin: "bottom" });
        }
